 ...
 */
int32 TraverseTable(lua_State *L, int32 Index, void *Userdata, TFunction<bool (lua_State*, void*)> Func)
{
    if (Index < 0 && Index > LUA_REGISTRYINDEX)
    {
        int32 Top = lua_gettop(L);
...
int32 Global_Printf(lua_State* L)
{
	FString StrLog;
	int32 nargs = lua_gettop(L);
	int32 flag = lua_tointeger(L, 1);
	for (int32 i = 2; i <= nargs; ++i)
	{
		const char* arg = luaL_tolstring(L, i, nullptr);
		if (!arg)
		{
	...
	if		(flag == 1) { UE_LOG(LogUnLua, Warning, TEXT("==== : %s"), *StrLog);  }
	else if (flag == 2) { UE_LOG(LogUnLua, Error, TEXT("==== : %s"), *StrLog); }
	else {				  UE_LOG(LogUnLua, Log, TEXT("==== : %s"), *StrLog); }
	if (IsInGameThread())
	{
		UKismetSystemLibrary::PrintString(GWorld, StrLog, false, false);
	}
