...
public:
    ...
    virtual bool SetValueInternal(lua_State *L, void *ValuePtr, int32 IndexInStack, bool bCopyValue) const override
    {
        ...
        if (Type == LUA_TTABLE)
        {
            ...
            TraverseTable(L, IndexInStack, &LuaArray, FArrayPropertyDesc::FillArray); // fill array elements
...
public:
    ...
    virtual bool SetValueInternal(lua_State *L, void *ValuePtr, int32 IndexInStack, bool bCopyValue) const override
    {
        ...
        if (Type == LUA_TTABLE)
        {
            ...
            TraverseTable(L, IndexInStack, &LuaMap, FMapPropertyDesc::FillMap); // fill map elements
...
public:
    ...
    virtual bool SetValueInternal(lua_State *L, void *ValuePtr, int32 IndexInStack, bool bCopyValue) const override
    {
        ...
        if (Type == LUA_TTABLE)
        {
            ...
            TraverseTable(L, IndexInStack, &LuaSet, FSetPropertyDesc::FillSet); // fill set elements
...
public:
		...
		: FStructPropertyDesc(InProperty), StructName(*FString::Printf(TEXT("F%s"), *StructProperty->Struct->GetName()))
	{
		...
		for (TFieldIterator<FProperty> It(StructProperty->Struct, EFieldIteratorFlags::ExcludeSuper, EFieldIteratorFlags::ExcludeDeprecated); It; ++It)
		{
			FProperty* FieldProperty = *It;
			FieldProperties.Emplace(StructProperty->Struct->GetAuthoredNameForField(FieldProperty), FPropertyDesc::Create(FieldProperty));
		}
	}
    ...
    virtual bool SetValueInternal(lua_State *L, void *ValuePtr, int32 IndexInStack, bool bCopyValue) const override
    {
		...
		{
			// fill struct fields
			TraverseTable(L, IndexInStack, nullptr, [=](lua_State* L, void* Userdata)
			{
				FString Key = UTF8_TO_TCHAR(lua_tostring(L, -2));
				const TSharedPtr<UnLua::ITypeInterface>* FieldProperty = FieldProperties.Find(Key);
				if (FieldProperty)
				{
					(*FieldProperty)->Write(L, ValuePtr, -1);
				}
				return true;
			});
		}
...
private:
    TMap<FString, TSharedPtr<UnLua::ITypeInterface>> FieldProperties;
