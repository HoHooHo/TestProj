...
void FSMTransitionEdgeCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	AvailableDelegates.Add(MakeShareable(new FString(TEXT("None"))));
	...
	TSharedPtr<FString> SelectedDelegateItem;
	FString CurDelegatePropertyName = TransitionNode->DelegatePropertyName.ToString();
	for (TSharedPtr<FString>& AvailableDelegate : AvailableDelegates)
	{
		if (AvailableDelegate->Equals(CurDelegatePropertyName))
		{
			SelectedDelegateItem = AvailableDelegate;
			break;
		}
	}
	...
	if (IDetailPropertyRow* Row = DetailBuilder.EditDefaultProperty(DelegatePropertyName))
	{
		...
		[
			...
			[
				...
				.InitiallySelectedItem(SelectedDelegateItem)
				...
				{
					// When selecting a property from the drop down.
					if (SelectInfo != ESelectInfo::Direct && DelegatePropertyName->IsValidHandle())
					{
